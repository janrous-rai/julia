...
function abstract_call_gf_by_type(interp::AbstractInterpreter, @nospecialize(f),
    ...
    for i in 1:napplicable
        ...
        if splitunions
            splitsigs = switchtupleunion(spec_types)
...
function abstract_call_known(interp::AbstractInterpreter, @nospecialize(f),
        ...
        sv::InferenceState,
        max_methods::Int = InferenceParams(interp).MAX_METHODS)
    # we don't expect those functions handled here are overloaded nor customized by a plugin
    # we will infer them using `fargs′` and `argtypes′` that ignore a plugin context,
    # and we will get rid of the plugin context later
    isoverdub = is_overdubbing_call(argtypes)
